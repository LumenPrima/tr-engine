...
|----------|--------|-----------|
| Permanent | calls, call_frequencies, call_transmissions, unit_events, transcriptions, talkgroups, units, trunking_messages | Forever by default; calls + call_frequencies + call_transmissions + transcriptions are opt-in droppable via `RETENTION_CALLS` (whole-month partition drop, see below) |
