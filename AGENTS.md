...
- IMBE ASR integration — when `STT_PROVIDER=imbe`, transcription is enqueued only by `handleDvcf` (not `handleAudio` or other handlers). The DVCF plugin (`tr-plugin-dvcf`) publishes `.dvcf` files (SymbolStream v2 binary format) on a separate MQTT topic (`{topic}/dvcf`) without the standard TR envelope (no `instance_id`). The MQTT payload is `{"audio_dvcf_base64": "...", "metadata": {...}}` where metadata includes talkgroup info, signal quality (`signal`, `noise`, `freq_error`), call timing, emergency/priority flags, TDMA state, and `srcList`. `handleDvcf` saves the file, finds the matching call via `FindCallBySystemName(system_name, tgid, start_time ±10s)`, and enqueues transcription. All other `enqueueTranscription()` call sites are no-ops when IMBE is active. Backfill skips non-`.dvcf` paths when the provider is IMBE. **Known limitation:** IMBE only handles P25 digital calls with DVCF. Analog/conventional calls get no transcription. Dual-provider routing (`STT_FALLBACK_PROVIDER`) is a 1.0-blocker on the roadmap (open PR work in flight).
...
- `call_update` appears in older docs/OpenAPI but is **not** currently published by the ingest pipeline
- `Last-Event-ID` header for gapless reconnect (ring buffer; missing IDs fall back to replaying all buffered events)
...
- **Auth model (public demo):** `AUTH_TOKEN` + `ADMIN_PASSWORD` set (full mode). Dashboard fetches `read_token` from auth-init for guest read access. Caddy does not need to inject auth headers. Admins log in via JWT for write operations.
...
Auth header injection is obsolete. Dashboard and `auth.js` discover auth state with `GET /api/v1/auth-init`; in full mode, unauthenticated browsers receive the optional `read_token` from that endpoint. Caddy should just proxy `/api/*` to tr-engine and let clients send their own `Authorization` header when needed.

The `tr-engine.luxprimatech.com` direct-access domain also expects callers to provide their own token, JWT, or API key when auth is enabled.
...
The trunk-recorder call ID format `{sys_num}_{tgid}_{start_time}` embeds `start_time`, which can shift by 1-2 seconds between `call_start` and `call_end` messages. This causes the call_end handler to fail exact-match lookup. tr-engine works around this with fuzzy matching by `(tgid, start_time ± 10s)` in the active calls map.
