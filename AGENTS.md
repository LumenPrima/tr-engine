...
- IMBE ASR integration — when `STT_PROVIDER=imbe`, transcription for P25 is enqueued by `handleDvcf`. The DVCF plugin (`tr-plugin-dvcf`) publishes `.dvcf` files (SymbolStream v2 binary format) on a separate MQTT topic (`{topic}/dvcf`) without the standard TR envelope (no `instance_id`). The MQTT payload is `{"audio_dvcf_base64": "...", "metadata": {...}}` where metadata includes talkgroup info, signal quality (`signal`, `noise`, `freq_error`), call timing, emergency/priority flags, TDMA state, and `srcList`. `handleDvcf` saves the file, finds the matching call via `FindCallBySystemName(system_name, tgid, start_time ±10s)`, and enqueues to the primary (IMBE) pool. **Dual-provider routing:** set `STT_FALLBACK_PROVIDER=whisper` (or `elevenlabs`/`deepinfra`) so analog/audio jobs from `enqueueTranscription()` go to a second worker pool. Routing is by path: `.dvcf` → primary IMBE, anything else → fallback. Without a fallback, non-DVCF enqueue sites remain no-ops when IMBE is primary.
