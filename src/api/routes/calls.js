...
// GET /calls - handles both active and historical calls
router.get('/', async (req, res) => {
    try {
        const isActive = req.baseUrl.includes('/active');
        
        if (isActive) {
            // Handle active calls request
            const filter = {};
            ...
            // Handle historical calls request
            const options = {
        ...
        logger.error('Error getting calls:', err);
        res.status(500).json({
            status: 'error',
            message: `Failed to retrieve ${req.baseUrl.startsWith('/api/v1/active') ? 'active' : 'historical'} calls`
        });
    }
});
    ...
    try {
        ...
        res.json({
            ...
            data: {
                talkgroup: {
                    id: talkgroupId,
                    tag: calls[0]?.payload?.call?.talkgroup_tag // Include tag if available from payload
                },
                time_range: {
                    start: options.startTime,
                    end: options.endTime
...
// GET /events
// Get currently active events (calls, emergencies, affiliations, system events)
