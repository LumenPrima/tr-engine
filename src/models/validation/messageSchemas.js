...
const audioMessageSchema = baseMessageSchema.keys({
    call: Joi.object({
        metadata: Joi.object({
            ...
            audio_status: Joi.string().valid('pending', 'recording', 'complete', 'failed', 'encrypted').default('pending')
        }).required(),
        audio_wav_base64: Joi.string(), // Audio data optional
...
const callStartSchema = baseMessageSchema.keys({
    call: Joi.object({
        ...
        freq: Joi.number(),
        audio_type: Joi.string(),
        audio_status: Joi.string().valid('pending', 'recording', 'complete', 'failed', 'encrypted', 'none').default('pending')
    }).required()
});
