...
type TestFixtures struct {
	...
	// Talkgroups (same tgid exists in both systems to test sysid scoping)
	MetroPDMainID   int // sysid:348, tgid:9178
	CountyPDMainID  int // sysid:15a, tgid:9178
	MetroFireID     int // sysid:348, tgid:9200
	MetroEMSID      int // sysid:348, tgid:9300

	// Units
	MetroUnit1ID  int // sysid:348, unit_id:1001234
	MetroUnit2ID  int // sysid:348, unit_id:1001235
	CountyUnit1ID int // sysid:15a, unit_id:1001234 (same unit_id, different sysid)
...
func SeedTestData(t testing.TB, db *TestDB) *TestFixtures {
	...
	db.MustExec(t, `INSERT INTO talkgroups (id, sysid, tgid, alpha_tag, description, tg_group, tag, priority, mode)
		VALUES (1, '348', 9178, 'Metro PD Main', 'Metro Police Dispatch', 'Law Enforcement', 'Law Dispatch', 1, 'D')`)
	db.MustExec(t, `INSERT INTO talkgroups (id, sysid, tgid, alpha_tag, description, tg_group, tag, priority, mode)
		VALUES (2, '15a', 9178, 'County PD Main', 'County Police Dispatch', 'Law Enforcement', 'Law Dispatch', 1, 'D')`)
	db.MustExec(t, `INSERT INTO talkgroups (id, sysid, tgid, alpha_tag, description, tg_group, tag, priority, mode)
		VALUES (3, '348', 9200, 'Metro Fire', 'Metro Fire Dispatch', 'Fire', 'Fire Dispatch', 2, 'D')`)
	db.MustExec(t, `INSERT INTO talkgroups (id, sysid, tgid, alpha_tag, description, tg_group, tag, priority, mode)
		VALUES (4, '348', 9300, 'Metro EMS', 'Metro EMS Operations', 'EMS', 'EMS Dispatch', 3, 'D')`)
	f.MetroPDMainID = 1
	f.CountyPDMainID = 2
	f.MetroFireID = 3
	f.MetroEMSID = 4
	...
	db.MustExec(t, `INSERT INTO talkgroup_sites (talkgroup_id, system_id) VALUES (1, 1), (2, 2), (3, 1), (4, 1)`)

	// Units - note: same unit_id (1001234) exists in both systems
	db.MustExec(t, `INSERT INTO units (id, sysid, unit_id, alpha_tag, alpha_tag_source)
		VALUES (1, '348', 1001234, 'Metro Car 123', 'radioreference')`)
	db.MustExec(t, `INSERT INTO units (id, sysid, unit_id, alpha_tag, alpha_tag_source)
		VALUES (2, '348', 1001235, 'Metro Car 124', 'radioreference')`)
	db.MustExec(t, `INSERT INTO units (id, sysid, unit_id, alpha_tag, alpha_tag_source)
		VALUES (3, '15a', 1001234, 'County Unit 1', 'manual')`)
	f.MetroUnit1ID = 1
	f.MetroUnit2ID = 2
	f.CountyUnit1ID = 3

	// Link units to systems
	db.MustExec(t, `INSERT INTO unit_sites (unit_id, system_id) VALUES (1, 1), (2, 1), (3, 2)`)

	// Calls - mix of systems and talkgroups
	// Call 1: Metro PD Main, 30 seconds ago, with audio
	db.MustExec(t, `INSERT INTO calls (id, instance_id, system_id, talkgroup_id, tr_call_id, call_num,
		...
		VALUES (1, 1, 1, 1, '1705312200_850387500_9178', 1001, $1, $2, 15.5,
	...
	db.MustExec(t, `INSERT INTO calls (id, instance_id, system_id, talkgroup_id, tr_call_id, call_num,
		...
		call_state, mon_state, phase2_tdma, tdma_slot, conventional, analog, audio_type,
		freq, freq_error, error_count, spike_count, signal_db, noise_db)
		VALUES (2, 1, 1, 1, '1705312100_850387500_9178', 1002, $1, $2, 22.3,
	...
	db.MustExec(t, `INSERT INTO calls (id, instance_id, system_id, talkgroup_id, tr_call_id, call_num,
		...
		call_state, mon_state, phase2_tdma, tdma_slot, conventional, analog, audio_type,
		freq, freq_error, error_count, spike_count, signal_db, noise_db)
		VALUES (3, 1, 1, 3, '1705312000_851000000_9200', 1003, $1, $2, 45.0,
	...
	db.MustExec(t, `INSERT INTO calls (id, instance_id, system_id, talkgroup_id, tr_call_id, call_num,
		...
		call_state, mon_state, phase2_tdma, tdma_slot, conventional, analog, audio_type,
		freq, freq_error, error_count, spike_count, signal_db, noise_db)
		VALUES (4, 1, 2, 2, '1705311900_852000000_9178', 1004, $1, $2, 18.7,
	...
	db.MustExec(t, `INSERT INTO calls (id, instance_id, system_id, talkgroup_id, tr_call_id, call_num,
		...
		call_state, mon_state, phase2_tdma, tdma_slot, conventional, analog, audio_type,
		freq, freq_error, error_count, spike_count, signal_db, noise_db)
		VALUES (5, 1, 1, 1, '1705311800_850387500_9178', 1005, $1, $2, 10.0, true, false,
		0, 0, false, 0, false, false, 'digital',
	...
	// Transmissions for Call 1
	db.MustExec(t, `INSERT INTO transmissions (id, call_id, unit_id, unit_rid, start_time, duration, position, emergency, error_count, spike_count)
		VALUES (1, 1, 1, 1001234, $1, 5.0, 0.0, false, 0, 0)`, f.BaseTime.Add(-30*time.Second))
	db.MustExec(t, `INSERT INTO transmissions (id, call_id, unit_id, unit_rid, start_time, duration, position, emergency, error_count, spike_count)
		VALUES (2, 1, 2, 1001235, $1, 4.5, 5.5, false, 0, 0)`, f.BaseTime.Add(-24*time.Second))
	db.MustExec(t, `INSERT INTO transmissions (id, call_id, unit_id, unit_rid, start_time, duration, position, emergency, error_count, spike_count)
		VALUES (3, 1, 1, 1001234, $1, 5.0, 10.5, false, 0, 0)`, f.BaseTime.Add(-19*time.Second))

	// Unit events
	db.MustExec(t, `INSERT INTO unit_events (id, instance_id, system_id, unit_id, unit_rid, event_type, talkgroup_id, tgid, time)
		VALUES (1, 1, 1, 1, 1001234, 'call', 1, 9178, $1)`, f.BaseTime.Add(-30*time.Second))
	db.MustExec(t, `INSERT INTO unit_events (id, instance_id, system_id, unit_id, unit_rid, event_type, talkgroup_id, tgid, time)
		VALUES (2, 1, 1, 2, 1001235, 'call', 1, 9178, $1)`, f.BaseTime.Add(-24*time.Second))
	db.MustExec(t, `INSERT INTO unit_events (id, instance_id, system_id, unit_id, unit_rid, event_type, talkgroup_id, tgid, time)
		VALUES (3, 1, 1, 1, 1001234, 'join', 1, 9178, $1)`, f.BaseTime.Add(-60*time.Second))
	...
	db.MustExec(t, `SELECT setval('talkgroups_id_seq', 100)`)
	db.MustExec(t, `SELECT setval('units_id_seq', 100)`)
