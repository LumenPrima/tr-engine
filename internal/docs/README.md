...
docker run -d -p 8080:8080 \
  ...
  ghcr.io/lumenprima/tr-engine:0.3.1-beta3 --easy
```
...
docker run --rm \
  -v ./data:/data \
  -v ./config:/app/config \
  ghcr.io/lumenprima/tr-engine:0.3.1-beta3
...
**Note:** Beta releases require the specific version tag (e.g., `:0.3.1-beta3`). The `:latest` tag is only applied to stable releases.
