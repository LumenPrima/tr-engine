...
docker run --rm \
  -v ./data:/data \
  -v ./config:/app/config \
  ghcr.io/lumenprima/tr-engine:0.2.2-beta1
...
**Note:** Beta releases require the specific version tag (e.g., `:0.2.2-beta1`). The `:latest` tag is only applied to stable releases.
